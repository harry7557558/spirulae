...
const builtinFunctions = [
    ...
    ["Spheres", "(0.9+sin(2x)sin(2y)sin(2z))e^(x+y)"],
    ["Noisy Sphere", "x^2+y^2+z^2=1+0.1sin(10x)sin(10y)sin(10z)"],
    ["Noisy Octa", "abs(x)+abs(y)+abs(z)-1+0.7cos(10x)cos(10y)cos(10z)"],
    ["Noisy Peanut", "1.4-1/((x-1)^2+y^2+z^2)-1/((x+1)^2+y^2+z^2)=0.02(cos(30x)+cos(30y)cos(30z))"],
    ...
    ["Eyes 1", "n=3ln((x^2+z^2)/(|x|+0.01));10y^2+x^2+0.5z^2-0.3z=sqrt(x^2+z^2)sin(n)^2"],
    ["Eyes 2", "a=3(z+x+1);b=3(z-x+1);(3-2z)/9+((2x^2+z^2)/6)^3+100y^2=sin(min(a*sin(b),b*sin(a)))-cos(max(a*cos(b),b*cos(a)))"],
    ...
    ["Spiral 5", "k=0.14;r=hypot(x,y,0.01);r1=1/k*ln(r);10((k(xcos(r1)+ysin(r1))-(0.5r)^2)^2+((z+0.5r-0.5)(r^2+0.1))^2)=r^2"],
    ...
    ["Skeleton", "f(x,y,z)=20(cos(x)sin(y)+cos(y)sin(z)+cos(z)sin(x))-(cos(2x)cos(2y)+cos(2y)cos(2z)+cos(2z)cos(2x))-28;-f(4x,4y,4z)"],
    ...
    ["Mandelbrot", "c=(x-1/2)+hypot(y,z)i;f(z1)=z1^2+c;g=f(f(f(f(f(f(c;log(|g|)=0"],
...
vec4 {%funname%}C(float x, float y, float z) {\n\
    ...
    initParameters([
        ...
        new GraphingParameter("sColor", "select-color",
            function(value) {
                if (value == 0) {
                    Module.ccall('setColorSource', null, ['string'], [""]);
                    return;
                }
                function fun() {
                    let shaderSource = getShaderSource("color.glsl");
                    if (shaderSource === null) {
                        setTimeout(fun, 10);
                        return;
                    }
                    shaderSource = shaderSource.replaceAll("{%COLOR%}", value);
                    Module.ccall('setColorSource', null, ['string'], [shaderSource]);
                }
                fun();
            }),
...
window.ModelExporter = {
    ...
    downloadSTL: function() {
        ...
        ModelExporter.downloadFile(ptr, 'meshgen3.stl', 'model/stl');
    },
    downloadPLY: function() {
        if (ModelExporter.assertModelNonempty()) return;
        let ptr = Module.ccall('generatePLY', 'int', [], []);
        // ModelExporter.downloadFile(ptr, 'meshgen3.ply', 'application/octet-stream');
        ModelExporter.downloadFile(ptr, 'meshgen3.ply', 'model/stl');
    },
    downloadOBJ: function() {
        if (ModelExporter.assertModelNonempty()) return;
        let ptr = Module.ccall('generateOBJ', 'int', [], []);
        ModelExporter.downloadFile(ptr, 'meshgen3.obj', 'model/obj');
    },
    downloadGLB: function() {
        if (ModelExporter.assertModelNonempty()) return;
        let ptr = Module.ccall('generateGLB', 'int', [], []);
        ModelExporter.downloadFile(ptr, 'meshgen3.glb', 'model/gltf-binary');
    },
    init: function() {
